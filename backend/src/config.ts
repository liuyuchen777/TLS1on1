...
if (
  ...
  console.log(
    `ENV -> port: ${process.env.PORT}, connection string: ${process.env.DB_CONNECTION_STRING}`
  );
  console.log(
    `ENV -> default password: ${process.env.DEFAULT_PASSWORD}, jwt secret: ${process.env.JWT_SECRET}`
  );
}
